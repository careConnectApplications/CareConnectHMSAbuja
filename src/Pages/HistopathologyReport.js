...
import { GetAllHistopathologyReportApi } from "../Utils/ApiCalls";
...
export default function HistopathologyReport() {
    ...
    const getAllLabReport = async () => {
        setIsLoading(true)
        try {
            const result = await GetAllHistopathologyReportApi();
            console.log("getAllHistopathologyReport", result)
    ...
    const ViewLabResult = (id) => {
        nav(`/dashboard/histopathology-process/report/${id}`)
        ...
        <MainLayout>
         {
            ...
            <Seo title="Histopathology Report" description="Care connect Manage Histopathology Report" />
