...
import {
  UpdatePhysiotherapyAssessmentApi,
  SettingsApi,
  GetDiagnosisICApi,
  GetAllPhysiotherapyAssessmentByPatientApi,
} from "../Utils/ApiCalls";
...
export default function EditPhysiotherapyAssessment() {
  ...
  useEffect(() => {
    ...
    // Check if any data exists to enable the Update button
    const hasData =
  ...
  const loadAssessment = async () => {
    try {
      ...
      const storedAssessment = JSON.parse(
        localStorage.getItem("PhysiotherapyAssessment")
      );

      if (storedAssessment) {
        setAssessmentId(storedAssessment._id || "");

        // Clinical Assessment
        setChiefComplaint(
          storedAssessment.clinicalassessment?.chiefComplaint || []
        );
        setHistoryOfPresentCondition(
          storedAssessment.clinicalassessment?.historyOfPresentCondition || []
        );
        setMedicalHistory(
          storedAssessment.clinicalassessment?.medicalHistory || []
        );
        setSurgicalHistory(
          storedAssessment.clinicalassessment?.surgicalHistory || []
        );
        setMedications(storedAssessment.clinicalassessment?.medications || []);
        setPreviousTreatments(
          storedAssessment.clinicalassessment?.previousTreatments || []
        );
        ...
        setPayload((prev) => ({
          ...prev,
          bloodPressure:
            storedAssessment.physicalexamination?.bloodPressure || "",
          pulse: storedAssessment.physicalexamination?.pulse || "",
          respiratoryRate:
            storedAssessment.physicalexamination?.respiratoryRate || "",
          temperature: storedAssessment.physicalexamination?.temperature || "",
        }));
        setMuscleStrengthTesting(
          storedAssessment.physicalexamination?.muscleStrengthTesting || []
        );
        setPosturalAssessment(
          storedAssessment.physicalexamination?.posturalAssessment || []
        );
        setGaitAnalysis(
          storedAssessment.physicalexamination?.gaitAnalysis || []
        );
        setPalpationFindings(
          storedAssessment.physicalexamination?.palpationFindings || []
        );
        setSpecialTests(
          storedAssessment.physicalexamination?.specialTests || []
        );
        ...
        setPayload((prev) => ({
          ...prev,
          visualAnalogScaleForPain:
            storedAssessment.outcomeMeasures?.visualAnalogScaleForPain || "",
          oswestryDisabilityIndex:
            storedAssessment.outcomeMeasures?.oswestryDisabilityIndex || "",
          timeUpAndGoTest:
            storedAssessment.outcomeMeasures?.timeUpAndGoTest || "",
          sixMinutesWalkTest:
            storedAssessment.outcomeMeasures?.sixMinutesWalkTest || "",
        }));
        setOutcomeMeasureNotes(
          storedAssessment.outcomeMeasures?.outcomeMeasureNotes || []
        );

        // Diagnosis and Clinical Impression
        setDiagnosisICDEleven(
          storedAssessment.diagnosisandclinicalImpression?.diagnosisICDEleven ||
            []
        );
        setPrimaryDiagnosisNotes(
          storedAssessment.diagnosisandclinicalImpression
            ?.primaryDiagnosisNotes || []
        );
        setSecondaryDiagnosisNotes(
          storedAssessment.diagnosisandclinicalImpression
        ...
        setShortTermGoals(
          storedAssessment.treatmentplanandgoals?.shortTermGoals || []
        );
        setLongTermGoals(
          storedAssessment.treatmentplanandgoals?.longTermGoals || []
        );
        setIntervention(
          storedAssessment.treatmentplanandgoals?.intervention || []
        );
        ...
        setPayload((prev) => ({
          ...prev,
          affectedBodyPart:
            storedAssessment.rangeofmotion?.affectedBodyPart || "",
          slideOfBody: storedAssessment.rangeofmotion?.slideOfBody || "",
          jointName: storedAssessment.rangeofmotion?.jointName || "",
          movementType: storedAssessment.rangeofmotion?.movementType || "",
          activeRangeOfMotion:
            storedAssessment.rangeofmotion?.activeRangeOfMotion || "",
          passiveRangeOfMotion:
            storedAssessment.rangeofmotion?.passiveRangeOfMotion || "",
          normalRangeOfMotion:
            storedAssessment.rangeofmotion?.normalRangeOfMotion || "",
          ROMDeficit: storedAssessment.rangeofmotion?.ROMDeficit || "",
          painLevelDuringMovement:
            storedAssessment.rangeofmotion?.painLevelDuringMovement || "",
          endFeel: storedAssessment.rangeofmotion?.endFeel || "",
          assessmentToolUsed:
            storedAssessment.rangeofmotion?.assessmentToolUsed || "",
          functionalImpact:
            storedAssessment.rangeofmotion?.functionalImpact || "",
          progressNotes: storedAssessment.rangeofmotion?.progressNotes || "",
        }));
        setNotes(storedAssessment.rangeofmotion?.notes || []);

        // Functional Limitations
        setFunctionalLimitations(storedAssessment.functionalLimitations || []);
      ...
      activateNotifications("Failed to load assessment data", "error");
    }
  };
  ...
  const handleCompleted = async () => {
    ...
    // Prepare the payload according to your structure
    const updatePayload = {
    ...
    try {
      const result = await UpdatePhysiotherapyAssessmentApi(
        AssessmentId,
        updatePayload
      );
      ...
      if (result.status === 200) {
        console.log("Update successful:", result.data); // Log success data
        setLoadingCompleted(false);
        activateNotifications(
          "Physiotherapy Assessment Updated Successfully. Redirecting...",
          "success"
        );
        setTimeout(() => {
        ...
        console.error("Update failed:", result); // Log error details
        setLoadingCompleted(false);
        activateNotifications(
          "Failed to update physiotherapy assessment.",
          "error"
        );
      }
    } catch (e) {
      console.error("API Error:", e); // Log the error
      console.error("Error details:", e.response?.data); // Log response data if available
      setLoadingCompleted(false);
      activateNotifications(
        e.message || "An error occurred while updating the assessment",
        "error"
      );
    }
  };
  ...
  return (
    <MainLayout>
      ...
      <Seo
        title="Edit Physiotherapy Assessment"
        description="Edit Physiotherapy Assessment"
      />
      <Box>
        <Button
