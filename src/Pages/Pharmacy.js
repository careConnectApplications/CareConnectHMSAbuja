...
export default function Pharmacy() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        // Use the GroupReadAllPharmacyApi here
        const data = await GroupReadAllPharmacyApi();
        console.log("Group API response:", data);
        // Assuming the new API returns data.queryresult as an array of objects
        setOriginalData(data.queryresult);
        setFilterData(data.queryresult);
  ...
  const filterBy = (field) => {
    if (field === "patient") {
      ...
      const filtered = OriginalData.filter((item) => {
        const prescriber = String(item.prescribersname || "").toLowerCase();
        return prescriber.includes(SearchInput.toLowerCase());
      });
      setFilteredData(filtered);
    } else if (field === "mrn") {
      const filtered = OriginalData.filter((item) => {
      ...
      // ← NEW LOGIC HERE
      const filtered = OriginalData.filter((item) => {
        const orderIdValue = String(item.orderid || "").toLowerCase();
        return orderIdValue.includes(SearchInput.toLowerCase());
      });
      setFilteredData(filtered);
    } else if (field === "date") {
  ...
  const filterPaid = () => {
    ...
    const filtered = OriginalData.filter(
      (item) => item.payment?.status?.toLowerCase() === "paid"
    );
  ...
  const filterPending = () => {
    ...
    setDispensed(false);
    const filtered = OriginalData.filter(
      (item) => item.payment?.status?.toLowerCase() === "pending payment"
    );
    setFilterData(filtered);
  };
  ...
  const filterDispensed = () => {
    ...
    setDispensed(true);
    const filtered = OriginalData.filter(
      (item) => item.dispensestatus?.toLowerCase() === "complete"
    );
    setFilterData(filtered);
  };
