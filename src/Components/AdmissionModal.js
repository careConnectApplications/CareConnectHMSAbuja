...
export default function AdmissionModal({ isOpen, onClose, setOldPayload, activateNotifications, oldPayload }) {
    ...
    const save = async () => {
        ...
        try {
            let result = await AdmitPatientApi(Payload, oldPayload.appointmentid)
            ...
            <ModalContent maxW={{ base: "90%", md: "60%" }} maxH="80vh"
                ...
                <ModalBody>
                    ...
                    <SimpleGrid mt="12px" mb="5" columns={{ base: 1, md: 2, lg: 2 }} spacing={5}>
                        ...
                        <Select h="45px" borderWidth="2px" fontSize={Payload.admittospecialization !== "" ? "16px" : "13px"} borderColor="#6B7280" id="admittospecialization" value={Payload.admittospecialization} onChange={handlePayload} placeholder="Admit to specialization" >
                            {
                        ...
                        <Select h="45px" borderWidth="2px" fontSize={Payload.referedward !== "" ? "16px" : "13px"} borderColor="#6B7280" id="referedward" value={Payload.referedward} onChange={handlePayload} placeholder="Select Referred Ward" >
                            {
                                Data?.map((item, i) => (
