...
export default function CreateWardModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} isCentered size="lg">
      ...
      <ModalContent>
        <ModalHeader>
          {type === "new" ? "Add New Ward" : "Edit Ward"}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {type === "new" ? (
            <>
              <Stack spacing="15px">
                ...
                <Input
                  val={Payload.wardname !== "" ? true : false}
                ...
                />
                <Input
                  val={Payload.totalbed !== "" ? true : false}
                  leftIcon={<FaBed />}
                  onChange={handlePayload}
                  type="number"
                  id="totalbed"
              ...
              <Button mt="32px" isLoading={Loading} onClick={AddWard}>
                Add Ward
              </Button>
            </>
          ) : (
            <>
              <Stack spacing="15px">
                <Select
                  ...
                  border="2px solid"
                  fontSize={
                    UpdatedPayload.bedspecialization !== "" ? "16px" : "13px"
                  }
                  borderColor="gray.500"
                >
                  {Settings?.clinics?.map((item, i) => (
                ...
                <Input
                  val={UpdatedPayload.wardname !== "" ? true : false}
                  leftIcon={<FaClinicMedical />}
                  onChange={handleUpdatedPayload}
                  id="wardname"
                  value={UpdatedPayload.wardname}
                  label="Ward Name"
                />
                <Input
                  val={UpdatedPayload.totalbed !== "" ? true : false}
                  leftIcon={<FaBed />}
                  onChange={handleUpdatedPayload}
                  type="number"
                  id="totalbed"
                  value={UpdatedPayload.totalbed}
                  label="Total bed"
                />
              </Stack>
              <Button mt="32px" isLoading={Loading} onClick={UpdateWard}>
                Update Ward
              </Button>
            </>
          )}
