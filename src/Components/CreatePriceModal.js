...
export default function CreatePriceModal({
  ...
  const AddPrice = async () => {
    setLoading(true);
    try {
      // Send category as "category" in the payload
  ...
  const UpdatePrice = async () => {
    ...
    try {
      // Send category as "category" in the payload
      const result = await UpdatePriceSettingAPI(
        UpdatedPayload,
        oldPayload._id
      );
  ...
  useEffect(() => {
    ...
    if (oldPayload) {
      setUpdatedPayload({
        ...
        isHMOCover: oldPayload.isHMOCover || "",
      });
    }
  }, [isOpen, oldPayload]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} isCentered size="lg">
      ...
      <ModalContent>
        <ModalHeader>
          {type === "new" ? "Add new Price" : "Edit Price"}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {type === "new" ? (
            <>
                ...
                {Payload.servicecategory !== "" && (
                  ...
                  >
                    {Settings?.servicecategory
                      ?.filter(
                        (item) => item.category === Payload.servicecategory
                      )[0]
                      ?.type?.map((item, i) => (
                        <option key={i} value={item}>
                          {item}
              ...
              <Button mt="32px" isLoading={Loading} onClick={AddPrice}>
                Add Price
              </Button>
            </>
          ) : (
            <>
              <Stack spacing="15px">
                <Select
                  ...
                  size="lg"
                  fontSize={
                    UpdatedPayload.servicecategory !== "" ? "16px" : "13px"
                  }
                  borderColor="gray.500"
                >
                  {Settings?.servicecategory?.map((item, i) => (
                ...
                {UpdatedPayload.servicecategory !== "" && (
                  <Select
                    ...
                    border="2px solid"
                    id="servicetype"
                    value={UpdatedPayload.servicetype}
                    size="lg"
                    fontSize={
                      UpdatedPayload.servicetype !== "" ? "16px" : "13px"
                    }
                    borderColor="gray.500"
                  >
                    {Settings?.servicecategory
                      ?.filter(
                        (item) =>
                          item.category === UpdatedPayload.servicecategory
                      )[0]
                      ?.type?.map((item, i) => (
                        <option key={i} value={item}>
                          {item}
                ...
                <Input
                  val={UpdatedPayload.servicetype !== ""}
                  leftIcon={<FaMoneyBill />}
                  onChange={handleUpdatedPayload}
                  id="amount"
                  value={UpdatedPayload.amount}
                  label="Amount"
                />
              ...
              <Button mt="32px" isLoading={Loading} onClick={UpdatePrice}>
                Update Price
              </Button>
            </>
          )}
