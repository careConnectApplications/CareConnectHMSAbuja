...
import { FaPills, FaSyringe, FaRoad } from "react-icons/fa";
import { MdNote } from "react-icons/md";
import { AiOutlineClockCircle } from "react-icons/ai";

export default function SingleMedicalChartModal({
  isOpen,
  onClose,
  admissionId,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast({
        status: "error",
        message: `Failed to ${type === "edit" ? "update" : "create"} medical chart: ${error.message}`,
      });
      onClose();
    } finally {
      setLoading(false);
