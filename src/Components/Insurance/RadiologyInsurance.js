...
export default function RadiologyInsurance() {
  ...
  const getAwaitingRadiology = async () => {
    setIsLoading(true);
    try {
      const result = await GetAwaitingAuthorizationRadiology();

      console.log("getAwaitingRadiology",result);
      if (result.status === true) {
        setIsLoading(false);
        setData(result.queryresult.radiologydetails);
        setFilterData(result.queryresult.radiologydetails);
      }
    } catch (e) {
